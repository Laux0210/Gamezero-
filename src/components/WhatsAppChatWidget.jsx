...
export function WhatsAppChatWidget() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div
          className="mb-3 w-[calc(100vw-32px)] sm:w-[380px] max-h-[580px] h-[520px] rounded-2xl border border-white/15 bg-background text-foreground shadow-2xl flex flex-col overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-bottom-5 duration-300"
          role="dialog"
          aria-label="Assistente Virtual GAME ZER0"
        >
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3.5 bg-surface border-b border-white/15">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-white/10 text-white/80">Oficial</span>
                </div>
                <p className="text-[11px] text-white/60">Resposta instantânea • +55 51 9204-2236</p>
              </div>
            </div>
            <button
              type="button"
          ...
          {/* Messages Area */}
          <div data-lenis-prevent className="flex-1 overflow-y-auto p-4 space-y-3.5 text-xs bg-background">
            {messages.map((m) => (
              <div
                key={m.id}
              ...
              >
                <div
                  className={`max-w-[85%] rounded-2xl p-3 shadow-sm ${
                    m.sender === "user"
                      ? "bg-surface text-white rounded-br-xs border border-white/15"
                      : "bg-secondary/60 text-white/85 rounded-bl-xs border border-white/10"
                  }`}
                >
                  ...
                  {m.options && (
                    <div className="mt-3 flex flex-col gap-1.5">
                      {m.options.map((opt) => (
                        ...
                        >
                          <span>{opt.label}</span>
                          <ArrowUpRight className="h-3 w-3 text-white/60 group-hover:text-white group-hover:translate-x-0.5 transition-transform" />
                        </button>
                      ))}
                    </div>
                  )}
                  ...
                  {m.cta && (
                    <div className="mt-3 pt-2.5 border-t border-white/10">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center justify-center gap-2 w-full py-2 px-3 bg-surface hover:bg-white hover:text-background text-white border border-white/15 rounded-xl font-medium text-xs transition-all shadow-sm group active:scale-[0.98]"
                      >
                        <Icon name="whatsapp" className="h-3.5 w-3.5 text-whatsapp" />
                        <span>{m.cta}</span>
                        <ArrowUpRight className="h-3 w-3 text-white/60 group-hover:text-background group-hover:translate-x-0.5 transition-transform" />
                      </a>
                    </div>
                  )}
                </div>
                <span className="text-[9px] text-white/45 mt-1 px-1 flex items-center gap-1">
                  {m.time}
                  {m.sender === "user" && <CheckCheck className="h-3 w-3 text-white/60" />}
                </span>
              </div>
            ))}

            {isTyping && (
              <div className="flex items-center gap-1.5 text-white/60 bg-secondary/60 border border-white/10 p-2.5 rounded-2xl rounded-bl-xs w-fit">
                ...
                <span className="text-[11px] ml-1 text-white/60">Digitando...</span>
              </div>
            )}
          ...
          <form
            ...
            className="p-2.5 bg-surface border-t border-white/15 flex items-center gap-2"
          >
            <input
              type="text"
              value={inputValue}
              ...
              className="flex-1 bg-background border border-white/15 rounded-xl px-3.5 py-2 text-xs text-white placeholder:text-white/45 focus:outline-none focus:border-white/50 transition-colors"
            />
            <button
              type="submit"
              disabled={!inputValue.trim()}
              className="p-2 bg-background hover:bg-white hover:text-background text-white disabled:opacity-40 disabled:hover:bg-background border border-white/15 rounded-xl transition-all shadow-sm active:scale-95"
      ...
      <button
        ...
        className="group relative flex items-center gap-2.5 px-4 py-3 bg-surface hover:bg-white hover:text-background text-white border border-white/15 hover:border-white/40 rounded-xl shadow-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_25px_rgba(255,255,255,0.08)] active:scale-[0.96] font-medium text-sm animate-float select-none cursor-pointer"
      ...
      >
        <div className="relative">
          ...
          {unread > 0 && !isOpen && (
            <span className="absolute -top-1.5 -right-1.5 h-3.5 w-3.5 rounded-full bg-primary text-background text-[9px] font-bold flex items-center justify-center animate-pulse">
              1
            </span>
          )}
        </div>
